...
def plot_dsd(
    ...
    # Calcular vmin y vmax si no se proporcionan
    if vmin is None:
    ...
    if log_scale:
        if vmin == 0.:
            # Es importante que vmin no sea 0 para escala logarítmica
            vmin = 0.1 # O un valor pequeño apropiado para tus datos
        norm = mpl.colors.LogNorm(vmin=vmin, vmax=vmax)
    else:
        norm = None
        # Eliminar pdb.set_trace() si no lo necesitas para depuración
        # import pdb
        # pdb.set_trace()

    plt.pcolormesh(
        np.array(dsd.time["data"]),
        np.array(dsd.diameter["data"]), # Asegúrate de que este también se convierta a np.array si es una lista
        dsd.fields["Nd"]["data"].T,
        # ELIMINAR vmin=vmin, vmax=vmax de aquí
        figure=fig,
        norm=norm,
        cmap=cmap,
        shading='auto' # Añadir shading='auto' para un mejor comportamiento
    )

    plt.axis("tight")

    # Es mejor usar ax.set_xlim y ax.set_ylim directamente para el objeto ax
    if xlims is not None:
        ax.set_xlim(xlims)
    else:
        # Asegúrate de que dsd.time["data"] sea un array de NumPy aquí también si es una lista
        time_data_array = np.array(dsd.time["data"])
        ax.set_xlim(time_data_array[0], time_data_array[-1])

    if ylims is not None:
        ax.set_ylim(ylims)
    else:
        # Asegúrate de que dsd.diameter["data"] sea un array de NumPy aquí también si es una lista
        diameter_data_array = np.array(dsd.diameter["data"])
        ax.set_ylim(0., diameter_data_array[-1])

    if tighten:
        diameter_data_array = np.array(dsd.diameter["data"])
        Nd_data_array = np.array(dsd.fields["Nd"]["data"])

        # Calcular la suma de concentración para cada diámetro
        sum_Nd_per_diameter = np.nansum(Nd_data_array, axis=0)

        # Encontrar los índices donde la concentración sumada es > 0
        valid_indices = np.where(sum_Nd_per_diameter > 0)[0]

        if len(valid_indices) > 0:
            # Si hay algún diámetro con datos, toma el índice máximo válido
            max_valid_diameter_index = np.max(valid_indices)
            max_diameter = diameter_data_array[max_valid_diameter_index]
        else:
            # Si no hay datos en ningún diámetro, establece max_diameter al diámetro máximo disponible
            # o a un valor por defecto que tenga sentido para tu visualización
            max_diameter = diameter_data_array[-1] # Último bin de diámetro
                                                    # o incluso diameter_data_array[0] si prefieres el más pequeño
                                                    # o un valor fijo como 5.0 mm si los datos están vacíos
        ax.set_ylim(0, max_diameter)

        # La lógica de tighten parece un poco compleja y podría necesitar ajuste
        # si los datos Nd tienen muchos NaNs o ceros.
        # np.nansum(Nd_data_array, axis=0) suma a lo largo del tiempo para cada diámetro
        # [::-1] invierte el array para buscar desde los diámetros más grandes
        # np.argmax(...) encuentra el índice del primer valor > 0
        # len(...) - ... encuentra el índice desde el inicio
        # Esto busca el diámetro máximo donde todavía hay datos significativos.
        max_diameter = diameter_data_array[
            len(diameter_data_array)
            - np.argmax(np.nansum(Nd_data_array, axis=0)[::-1] > 0)
        ]
        ax.set_ylim(0, max_diameter) # Usar ax.set_ylim

    plt.colorbar() # Esto creará una barra de color para el último pcolormesh
    plt.xlabel("Time(m)")
