...
class DropSizeDistribution(object):
    ...
    def __init__(self, reader, time_start=None, location=None):
        ...
        try: # AGREGADA MIA
            self.spectrum_fall_velocity = reader.fields["terminal_velocity"]
        except:
            self.spectrum_fall_velocity = None
...
0450(1969)008<0249:TVORA>2.0.CO;2, 1969.
    ...
    def calculate_RR(self):
        ...
        for t in range(0, self.numt):
            ...
            """
            self.fields["rain_rate"]["data"][t] = (
                np.pi()/6 * 3.6/10**3 * 1 / (eff[t] * 60) *
                    np.sum(self._mmultiply(
                        self.
                    )
    ...
    def calculate_dsd_from_spectrum(self, delta_t, velocity, effective_sampling_area=None, replace=True):
        ...
        delta_t: int
            Sampling interval in seconds or duration time of the event (in seconds).
        replace: boolean
        ...
        delta_t = delta_t #np.mean(np.diff(self.time["data"][0:4]))  # Sampling time in seconds
    ...
    def calculate_dsd_from_spectrum_columnwise(self, delta_t, vel_terminal, effective_sampling_area=None, replace=True):
        """
        Calcula N(D) desde drop_spectrum suponiendo que las matrices tienen los diámetros en columnas 
        y velocidades en filas, y que se usa una única velocidad por bin de diámetro (v_diam), como en la fórmula clásica.

        Parameters
        ----------
        delta_t : int
            Duracion del muestreo en segundos. Nota: si es un evento completo, va la duracion del mismo.
        vel_terminal : array
            Velocidad terminal asociada a cada bin de diámetro (shape: 32,)
        effective_sampling_area [mm]: function or array
            Área efectiva (en mm) por bin de diámetro (array o función de D). Si no se pasa, se intenta usar el atributo del objeto.
        delta_t : int
            Intervalo de muestreo en segundos (por ejemplo, 60)
        replace : bool
            Si True, reemplaza los datos en self.fields["Nd"]["data"], si False, los retorna sin modificar el objeto.
        """

        D = self.diameter["data"]

        # Obtener el área efectiva
        if effective_sampling_area is not None:
            if callable(effective_sampling_area):
                A = effective_sampling_area(D)
            else:
            ...
            A = filter.parsivel_sampling_area(D)
        #print("Eff que se esta usando es:", A)
        spread = self.spread["data"]
        data = self.fields["drop_spectrum"]["data"]  # shape: (T, 32, 32), velocidad en filas, diámetro en columnas

        # Sumar sobre filas (velocidad) para cada columna (diámetro)
        sum_nij = np.sum(data, axis=1)  # shape: (T, 32)
    
        # Calcular N(D)
        Nd = (1e6 * sum_nij) / (A * spread * delta_t * vel_terminal)  # broadcasting por columnas
        Nd = np.nan_to_num(Nd, nan=0.0)
        
        if replace:
            self.fields["Nd"]["data"] = Nd
            self.fields["Nd"]["source"] = "Calculated from spectrum (columnwise formula)."
        else:
            return Nd
    ...
    def _estimate_mu(self, idx):
        ...
        res = scipy.optimize.minimize_scalar(
            self._mu_cost, bounds=(-4, 15), args=(idx,), method="bounded" # LIMITES MODIFICADOS SEGUN CRITERIO
        )                                                                 # de CASANOVA
