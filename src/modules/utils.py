...
# ------------------------------------------------------------------------------
# Cleanup: pattern-based with dry-run and keep rules
# ------------------------------------------------------------------------------
...
def _is_under(base: Path, candidate: Path) -> bool:
    ...
    return cand == base or cand.is_relative_to(base)
...
def find_files(
    ...
    for dirpath, dirnames, filenames in os.walk(root_directory):
        ...
        if any(abs_dir.is_relative_to(ed) or abs_dir == ed for ed in ex_dirs):
            dirnames[:] = []
            continue
