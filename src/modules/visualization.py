...
# ---------------------------------------------------------------------
# Parameterized DSD Plots (from param_event_*.csv)
# ---------------------------------------------------------------------

def plot_dsd_parameters_timeseries(
    df: pd.DataFrame,
    event_id: Optional[int] = None,
    save_dir: Optional[Path] = None,
) -> Optional[plt.Figure]:
    """
    5-panel time-series plot of DSD physical parameters:
    Intensity, Dm, log10(Nw), LWC, Z_calc.
    Reads from a param_event_*.csv DataFrame.
    """
    try:
        df = df.copy()
        df["Datetime"] = pd.to_datetime(df["Datetime"], errors="coerce")
        df = df.dropna(subset=["Datetime"]).sort_values("Datetime")
        # Filter only computed rows
        if "status" in df.columns:
            df = df[df["status"] == "computed"]
        if df.empty:
            logger.info("No computed rows for DSD parameters plot.")
            return None

        t = df["Datetime"]
        panels = [
            ("Intensidad", "Intensity (mm/h)", "steelblue"),
            ("Dm", "Dm (mm)", "darkorange"),
            ("Nw", "log₁₀(Nw) (mm⁻¹m⁻³)", "green"),
            ("LWC", "LWC (g/m³)", "crimson"),
            ("Z_calc", "Z (dBZ)", "purple"),
        ]

        fig, axes = plt.subplots(len(panels), 1, figsize=(12, 14), sharex=True)
        label = f"Event {event_id}" if event_id else "Event"
        ...
        for ax, (col, ylabel, color) in zip(axes, panels):
            ...
            ax.plot(t, vals, "-o", color=color, markersize=2, linewidth=1)
            ax.set_ylabel(ylabel, fontsize=10)
            ax.grid(True, alpha=0.3)

        axes[0].set_title(f"DSD Parameters — {label}", fontsize=13, fontweight="bold")
        ...
        if save_dir:
            ensure_directory_exists(save_dir)
            fp = Path(save_dir) / f"dsd_params_event_{event_id or 'all'}.png"
            fig.savefig(fp, dpi=150)
            logger.info(f"Saved DSD parameters plot to {fp}")

        plt.close(fig)
        return fig
    except Exception:
        logger.exception("Error in plot_dsd_parameters_timeseries")
        return None


def plot_gamma_fit_params(
    df: pd.DataFrame,
    event_id: Optional[int] = None,
    save_dir: Optional[Path] = None,
) -> Optional[plt.Figure]:
    """
    3-panel time-series of Gamma DSD fit parameters:
    Mu, Lambda, log10(N0).
    """
    try:
        df = df.copy()
        ...
        if "status" in df.columns:
            df = df[df["status"] == "computed"]
        if df.empty:
            logger.info("No computed rows for Gamma fit plot.")
            return None
        ...
        # Mu
        if "Mu" in df.columns:
            ax1.plot(t, df["Mu"], "-o", color="teal", markersize=3, linewidth=1)
        ax1.set_ylabel("μ (shape)", fontsize=10)
        ax1.grid(True, alpha=0.3)
        ax1.set_title(f"Gamma Fit Parameters — {label}", fontsize=13, fontweight="bold")

        # Lambda
        if "Lambda" in df.columns:
            ax2.plot(t, df["Lambda"], "-s", color="coral", markersize=3, linewidth=1)
        ax2.set_ylabel("Λ (mm⁻¹)", fontsize=10)
        ax2.grid(True, alpha=0.3)

        # log10(N0)
        if "N0" in df.columns:
            n0_vals = df["N0"].values.astype(float)
            n0_log = np.where(n0_vals > 0, np.log10(n0_vals), np.nan)
            ax3.plot(t, n0_log, "-^", color="mediumpurple", markersize=3, linewidth=1)
        ax3.set_ylabel("log₁₀(N0)", fontsize=10)
        ax3.set_xlabel("Time")
        ...
        if save_dir:
            ensure_directory_exists(save_dir)
            fp = Path(save_dir) / f"gamma_fit_event_{event_id or 'all'}.png"
            fig.savefig(fp, dpi=150)
            logger.info(f"Saved Gamma fit plot to {fp}")

        plt.close(fig)
        return fig
    except Exception:
        logger.exception("Error in plot_gamma_fit_params")
        return None


def plot_dm_nw_scatter(
    df: pd.DataFrame,
    event_id: Optional[int] = None,
    save_dir: Optional[Path] = None,
) -> Optional[plt.Figure]:
    """
    Scatter plot of Dm vs log10(Nw).
    Overlays the stratiform/convective separation line from
    Bringi et al. (2003).
    """
    try:
        df = df.copy()
        ...
        if df.empty or "Dm" not in df.columns or "Nw" not in df.columns:
            logger.info("No data for Dm-Nw scatter.")
            return None

        dm = df["Dm"].values.astype(float)
        nw = df["Nw"].values.astype(float)
        log_nw = np.where(nw > 0, np.log10(nw), np.nan)

        label = f"Event {event_id}" if event_id else "Event"

        fig, ax = plt.subplots(figsize=(8, 7))
        ax.scatter(dm, log_nw, c="steelblue", s=30, alpha=0.7, edgecolors="white",
                   linewidths=0.5, label=label)

        # Bringi et al. (2003) separation line
        # Convective: log10(Nw) = -1.6 * Dm + 6.3
        # Stratiform: log10(Nw) = -1.6 * Dm + 6.3  (same line, different side)
        dm_line = np.linspace(0.5, 4.0, 50)
        nw_line = -1.6 * dm_line + 6.3
        ax.plot(dm_line, nw_line, "k--", linewidth=1.5, label="Bringi et al. (2003)")
        ax.text(1.0, 5.0, "Stratiform", fontsize=10, fontstyle="italic", color="gray")
        ax.text(2.5, 2.5, "Convective", fontsize=10, fontstyle="italic", color="gray")

        ax.set_xlabel("Dm (mm)", fontsize=12)
        ax.set_ylabel("log₁₀(Nw) (mm⁻¹ m⁻³)", fontsize=12)
        ax.set_title(f"Dm vs Nw — {label}", fontsize=13, fontweight="bold")
        ax.set_xlim(0, 4.5)
        ax.set_ylim(1, 7)
        ax.grid(True, alpha=0.3)
        ...
        if save_dir:
            ensure_directory_exists(save_dir)
            fp = Path(save_dir) / f"dm_nw_scatter_event_{event_id or 'all'}.png"
            fig.savefig(fp, dpi=150)
            logger.info(f"Saved Dm-Nw scatter to {fp}")

        plt.close(fig)
        return fig
    except Exception:
        logger.exception("Error in plot_dm_nw_scatter")
        return None
