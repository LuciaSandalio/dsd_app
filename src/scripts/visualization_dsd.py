...
from modules.visualization import (
    ...
    plot_dsd_parameters_timeseries,
    plot_gamma_fit_params,
    plot_dm_nw_scatter,
)
...
def main() -> None:
    ...
    for site_dir in sites:
        ...
        # --- Parameterized DSD Plots (from Stage 3 output) ---
        param_dir = SRC_ROOT.parent / "data" / "processed" / "event_parameters" / site
        if not param_dir.exists():
            param_dir = SRC_ROOT / "data" / "processed" / "event_parameters" / site
        
        if param_dir.exists():
            param_files = sorted(param_dir.glob("param_event_*.csv"))
            for pf in param_files:
                try:
                    p_eid = int(pf.stem.replace("param_event_", ""))
                except ValueError:
                    continue
                ...
                try:
                    ...
                    logger.warning(f"[{site}] Failed reading {pf.name}: {e}")
                    continue
                
                param_plot_dir = plots_root / f"Event_{p_eid}" / "parameters"
                ensure_directory_exists(param_plot_dir)
                
                plot_dsd_parameters_timeseries(pdf, event_id=p_eid, save_dir=param_plot_dir)
                plot_gamma_fit_params(pdf, event_id=p_eid, save_dir=param_plot_dir)
                plot_dm_nw_scatter(pdf, event_id=p_eid, save_dir=param_plot_dir)
                
                logger.info(f"[{site}] Parameterized plots saved for event {p_eid}")
        else:
            logger.info(f"[{site}] No parameterized data found at {param_dir}")
